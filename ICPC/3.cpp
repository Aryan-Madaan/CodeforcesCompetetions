 ...
 *  Created: 19-08-2021 at 17:10:02
 *
...
const int N = 1e5, mod = 1e9, inf = 1e18;
...
void solve()
{
    int k, d;
    cin>>k>>d;
    int nodes = ((((mpow(k, d) - 1)/(k-1)))%mod+1)%mod;
    // cout<<((((mpow(k, d) - 1 + mod)%mod)/(k-1)))%mod<<' '<<((((mpow(k, d-1) - 1 + mod)%mod)/(k-1)))%mod <<' ';
    cout<<(k*nodes/2)%mod<<endl;
}
